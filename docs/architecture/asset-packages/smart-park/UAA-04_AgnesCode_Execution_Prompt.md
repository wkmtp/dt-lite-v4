# DT-Lite V4.0 UAA-04 — External Integration & Mapping
...
实现 ExternalSystem、Connector、ConnectorInstance、ExternalObject、ExternalPoint、ExternalEvent、ExternalCommand、ExternalRelationship、IntegrationProfile、MappingProfile。

实现 Discovery → Classification → Mapping → Validation → Test。
...
[ ] BMS Mock Discovery PASS
[ ] ExternalObject → Asset PASS
[ ] ExternalPoint → Point PASS
[ ] ExternalEvent → Alarm/Event PASS
[ ] ExternalCommand → Capability PASS
[ ] ExternalRelationship → Relationship PASS
[ ] Factory MES/PLC Mock 可使用同一 Integration Contract
[ ] Core Contract 未修改
