# DT-Lite V4.0 UAA-07 — AI, Permission, Safety & Audit
...
实现 AITool、AIAgent、Permission、SafetyContract、AuditRecord。

强制：

Agent → Tool → Capability → Permission → Safety → Execution → Audit。

区分 READ_ONLY / MUTATING。
...
[ ] Read-only Tool 可执行
[ ] Mutating Tool 缺 Permission 时拒绝
[ ] 缺 SafetyContract 时拒绝
[ ] Agent 不能绕过 Tool
[ ] Agent 不能绕过 Capability
[ ] Agent 不能直接访问 DB
[ ] Agent 不能直接访问 Connector
[ ] Mutating execution 必须 Audit
[ ] AI Gate PASS
