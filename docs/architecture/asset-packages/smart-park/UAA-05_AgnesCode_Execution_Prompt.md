# DT-Lite V4.0 UAA-05 — Scene, BIM, GIS & 3D
...
实现 Model、ModelObject、ModelBinding、Scene、SceneTemplate、SceneBinding。

建立 ModelObject → Asset → Runtime State → 3D Visualization 链。

支持 BIM/GIS/GLB/glTF/3D Tiles 的来源抽象。
...
[ ] BIM Object 可绑定 Asset
[ ] GIS Object 可绑定 Asset
[ ] Asset Runtime State 可驱动 3D
[ ] 一个 Asset 可有多个 ModelBinding
[ ] 删除 ModelObject 不删除 Asset
[ ] Scene 引用 Asset 而非拥有 Asset
[ ] Factory Mock 可复用 Scene Contract
[ ] Spatial Gate PASS
