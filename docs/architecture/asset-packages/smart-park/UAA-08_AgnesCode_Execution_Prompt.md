# DT-Lite V4.0 UAA-08 — Zero-Code Assembly Engine
...
实现 AssemblyContext、AssemblyRecipe、AssemblyPlan、AssemblyStep、AssemblyValidation、AssemblyResult。

严格实现 Assembly State Machine、precondition、validation、retry、idempotency、rollback、audit、publish gate。

不得用 Smart Park hard-coded script 替代 Engine。
...
[ ] Assembly State Machine PASS
[ ] 非法 transition 被拒绝
[ ] Retry PASS
[ ] Idempotency PASS
[ ] Rollback PASS
[ ] Audit PASS
[ ] Publish 前 Validation + Acceptance
[ ] Runtime 不消费未 Published version
[ ] GS-10 所需装配可声明式表达
[ ] Zero-Code Gate PASS
